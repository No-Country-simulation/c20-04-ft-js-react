...
export default function EditProfileBasicInfoLayout({
  ...
  const validateForm = () => {
    if (
      dataToUpdate.newName !== "" &&
      (dataToUpdate.newName.trim().length < 3 ||
        dataToUpdate.newName.length > 20)
    ) {
      setErrorForm({
        ...errorForm,
        newName:
          "Name should contain at least 3 characters and a maximum of 20",
      });
    } else {
    ...
    if (
      dataToUpdate.newUsername !== "" &&
      (dataToUpdate.newUsername.length < 3 ||
        dataToUpdate.newUsername.length > 10)
    ) {
      setErrorForm({
        ...errorForm,
        newUsername:
          "Username should contain at least 3 characters and a maximum of 10",
      });
    } else {
  ...
  return (
    <div className="flex w-full flex-col mt-[1rem] items-center justify-center gap-6 p-6 bg-[#f8f9fa] rounded-lg shadow-md">
      {/* <p className="text-lg font-extrabold text-[#8c52ff] mb-4 text-center">
  Send the data you want to update
</p> */}
      ...
      <div className="flex flex-col gap-6 text-center lg:text-left lg:flex-row w-full justify-around items-center">
        ...
        <div className="flex flex-col items-center">
          <label
            htmlFor="newName"
            className="text-lg font-extrabold text-[#8c52ff] mb-2"
          >
            Your name: {name}
          </label>
          <input
        ...
        {/* Username Input */}
        <div className="flex flex-col items-center relative group mt-[1rem]">
          <div className="absolute w-[30px] h-[28px] overflow-hidden bottom-[100%] bg-white shadow-md rounded-md border border-gray-300 text-center transform transition-all duration-300 ease-in-out hover:w-[100px] hover:h-[140px] hover:bottom-10">
          <InfoOutlinedIcon className="text-red-600"/>
            <p className="hidden text-xs text-gray-600 italic group-hover:block">
              If you change the username, you will need to log in again.
            </p>
          </div>

          <label
            htmlFor="newUsername"
            className="text-lg text-center font-extrabold text-[#8c52ff] mb-2"
          >
            Username: {username}
          </label>
          <input
            ...
            onChange={(e) =>
              setDataToUpdate({
                ...dataToUpdate,
                newUsername: e.target.value.trim(),
              })
            }
      ...
      <div className="flex justify-center gap-[1rem] w-[90%] mx-auto mt-4 lg:max-w-[200px]">
        <button
          className="w-[100%] h-[40px] rounded max-w-[6rem] justify-self-start lg:min-w-[100px] border border-gray-300 text-gray-700 hover:bg-[#e2e5e9] transition-all duration-200"
          onClick={onClose}
        >
          Cancel
        </button>
